...
def default_source_train_npz(args):
    pair_tag = f"{args.source_session}_to_{args.target_session}"
    if args.stage2_example == "chewie_to_mihili":
        stage2_dir = REPO_ROOT / "outputs" / "Chewie_CO_2016" / "to_Mihili_CO_2014"
    else:
    ...
    return (
        ...
        / pair_tag
        / "eval"
        / "source_train_behavior_prediction_conditional_mmd.npz"
    )
...
def load_decoder_data(args):
    source_npz_path = args.source_train_npz or default_source_train_npz(args)
    target_npz_path = args.stage2_npz or default_stage2_npz(args)
    train_latents, train_behavior = load_npz_latents(source_npz_path, "source train")
    test_latents, test_behavior = load_npz_latents(target_npz_path, "target test")
    data_info = {
        "source_train_npz": str(source_npz_path),
        "target_test_npz": str(target_npz_path),
        "source_session_id": str(args.source_session),
        "target_session_id": str(args.target_session),
        "num_source_train_trials": int(train_latents.shape[0]),
        "num_target_test_trials": int(test_latents.shape[0]),
    }
    return train_latents, train_behavior, test_latents, test_behavior, data_info
...
def main():
    ...
    parser.add_argument("--source-train-npz", type=Path, default=None)
    ...
    train_latents, train_behavior, test_latents, test_behavior, data_info = load_decoder_data(args)
    ...
    x_test, y_true = build_lagged_decoder_matrices(test_latents, test_behavior, args.lag)
    y_pred = model.predict(x_test)
    r2_x, r2_y, mean_r2 = raw_and_mean_r2(y_true, y_pred)

    result = {
        "decoder": "lagged_ridge",
        "source_train_npz": data_info["source_train_npz"],
        "target_test_npz": data_info["target_test_npz"],
        "source_session_id": data_info["source_session_id"],
        "target_session_id": data_info["target_session_id"],
        "lag": int(args.lag),
        ...
        "r2_y": r2_y,
        "mean_r2": mean_r2,
        "num_source_train_trials": int(data_info["num_source_train_trials"]),
        "num_target_test_trials": int(data_info["num_target_test_trials"]),
    }
