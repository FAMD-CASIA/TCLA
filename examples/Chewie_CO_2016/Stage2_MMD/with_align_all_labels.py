...
from TCLA.losses import conditional_mmd_loss
...
stage2_config = example_config.stage2_conditional_mmd

SOURCE_SESSION = os.environ.get("SOURCE_SESSION_ID", example_config.experiment.source_session)
TARGET_SESSION = os.environ.get("TARGET_SESSION_ID", example_config.experiment.target_session)
PAIR_TAG = f"{SOURCE_SESSION}_to_{TARGET_SESSION}"


cfg = OmegaConf.create(
    {
        "model": OmegaConf.to_container(stage2_config.model, resolve=True),
        "dataset": {
            "all_sessions": [SOURCE_SESSION, TARGET_SESSION],
            "datapath_root": os.path.join(REPO_ROOT, "data", example_config.experiment.source_dataset),
            "signal_length": stage2_config.dataset.signal_length,
        },
        "adapt": OmegaConf.to_container(stage2_config.adapt, resolve=True),
        "exp": {
            "source_behavior_checkpoint": os.path.join(
                REPO_ROOT,
                "outputs",
                ...
                "model",
                "full_ae_behavior_model_final.pt",
            ),
...
def load_ae_state(model, state_dict):
...
def evaluate_behavior_r2(target_ae, behavior_head, data_loader, target_session_id, device):
    ...
    }, y_true_trials, y_pred_trials, z_latent_trials
...
def run_target_adaptation_conditional_mmd(cfg, target_session_id, source_session_id):
    ...
    all_session_data, all_session_configs = get_session_data_and_configs(
        cfg.dataset.all_sessions,
        cfg.dataset.datapath_root,
    )

    source_data = all_session_data[source_session_id]
    target_data = all_session_data[target_session_id]
    ...
    load_ae_state(target_ae, ckpt["ae_state_dict"])
    ...
    adaptation_params = freeze_all_but_read_adapters(target_ae, target_session_id)
    ...
    for epoch in pbar:
        ...
        for target_batch in target_train_loader:
            ...
            output_rates_tgt, z_target = target_ae(input_signal_tgt, target_session_id)
        ...
        with torch.no_grad():
            for target_batch in target_val_loader:
                ...
                output_rates_tgt, z_target = target_ae(signal_tgt, target_session_id)
    ...
    r2_result, y_true_trials, y_pred_trials, z_latent_trials = evaluate_behavior_r2(
        target_ae,
        behavior_head,
        target_test_loader,
        target_session_id,
